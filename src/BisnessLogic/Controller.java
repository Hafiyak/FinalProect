...
import Objects.Room;
import BisnessLogic.AbstractDAOImpl;
...
public class Controller {
    ...
    private String lastNameRequest = "\u001B[32m" + "\tPlease, enter your last name...";
    private String blankFieldErr = "\u001B[31m" + "\tField is blank or less then 4 symbols. Please, input again...";
    private String goodByeScreen = "\u001B[32m" + "\nThank you for using Book Online System!" +
            "\n=======================================";


    //Method used for scanning String input data
    private String getStringInput(String promtMessage, String errorMessage) {
    ...
    public void systemEnter() {
        ...
        abstractDAOImpl.addUsersToDataBase();

        System.out.println("\u001B[32m" + "\"Welcome to the Hotel Online Booking System!\" +\n" +
                "                \"\\n====================================\"");
        String name = getStringInput(nameRequest, blankFieldErr);
        String lastName = getStringInput(nameRequest, blankFieldErr);

        //Checking if user exists in the system DB already
        List<User> users = abstractDAOImpl.getUserDataBase()
                .stream()
                .filter(u -> u.getUserName().equalsIgnoreCase(name) && u.getUserLastName().equalsIgnoreCase(lastName))
                .collect(Collectors.toList());

        if (users.isEmpty()) {
            System.out.println("\u001B[31m" + "User does not exist. Please, register your account to enable search +" +
    ...
    private void createNewUser() {
        System.out.println("\tUser's sign up system" +
                "\n");
        String name = getStringInput(nameRequest, blankFieldErr);
        String lastName = getStringInput(nameRequest, lastNameRequest);
        User newUser = new User(findNewUserID(), name, lastName);        //Creating new User
        abstractDAOImpl.addNewUser(newUser);                          //Adding new User to the user list
    ...
    private  long findNewUserID() {
        return abstractDAOImpl.getUserDataBase().size() +1;
    }
    ...
    private void actionSelect(boolean isLoggedIn) {
        ...
        while (!choise.equals("1")&& !choise.equals("2")|| choise.isEmpty()) {
            ...
            choise = scanner.nextLine();
                ...
                case "1":
                    ...
                    findHotelByCity(hotelName, isLoggedIn);
                    break;
                case "2":
    ...
    private List<Hotel> findHotelByCity(String city, boolean isLoggedIn) {
        logInCheck(isLoggedIn);
        List<Hotel> foundedHotels = abstractDAOImpl.getHotels()
                .stream()
        ...
        if (!foundedHotels.isEmpty()) {
            ...
            List<Room> selectedRooms = new ArrayList<>();
            foundedHotels.forEach(c -> selectedRooms.addAll(c.getRooms()));
    ...
    private void bookRoom(long roomId, long userId, long hotelId, boolean isLoggedIn) {
        ...
        User registeredUser = abstractDAOImpl
                .getUserDataBase().stream().filter(c -> c.getUserId() == userId).findAny().get();
